...
if (isset($_SESSION['userid'])) {
    ...
    <body>
        ...
        <header class="main">
            ...
            $sql = "SELECT post_title, post_body, post_author, file_name FROM forum_post WHERE post_id=? AND forum_id=? AND post_type='o'";
            if ($topicPost = $db->prepare($sql)) {
                $topicPost->bind_param('ss', $pid, $id);
                $topicPost->bind_result($post_title, $post_body, $post_author, $file_name);
                $topicPost->execute();
            ...
            $sql1 = "SELECT forum_post.post_id, forum_post.post_body, forum_post.post_author, users.username
            FROM forum_post, users WHERE forum_post.original_id=? AND forum_post.post_type='r' AND forum_post.post_author = users.id 
                ...
                <table class="styled-table">
                    ...
                    <tr>
                        <form action="../server/server.php" method="post">
                            ...
                            <td><textarea placeholder="Enter a reply..." required minlength="2" maxlength="40" rows="2"
                                    name="reply-text"></textarea> <br> <button type="submit"
                                    name="post-reply">REPLY</button> </td>
                        </form>
                    </tr>
                </table>
                <br>
